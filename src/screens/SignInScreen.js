...
//     // ✅ ADMIN — no API call, direct navigate
//     if (
...
//       return; // ← never reaches signInApi
//     }

//     // ✅ Normal user — email format check
//     if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(enteredEmail)) {
//       setErrors(prev => ({ ...prev, email: 'Enter a valid email address' }));
//       return;
//     }

//     // ✅ Normal user — API call
//     try {
//       setLoading(true);
//       setErrors({ email: '', password: '' });
...
export default function SignInScreen({ navigation }) {
  ...
  // ── Admin Login — only yellow color change, no auto-fill ──
  const fillAdmin = () => {
...
const styles = StyleSheet.create({
  ...
  // ── Admin Button — active/selected state ──
  adminButtonActive: {
    borderColor: '#4DEBFF',
    backgroundColor: '#4DEBFF',
  },
